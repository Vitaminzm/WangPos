...
import com.symboltechshop.wangpos.adapter.CanclePayAdapter;
...
import com.symboltechshop.wangpos.view.TextScrollView;
			...
			case R.id.text_submit_order:
				...
				for(int i=0;i<payments.size();i++){
					if( payments.get(i).getType().equals(PaymentTypeEnum.HANDRECORDED.getStyletype()) ||
							payments.get(i).getType().equals(PaymentTypeEnum.STORERECORDED.getStyletype()) ||payments.get(i).getType().equals(PaymentTypeEnum.WECHATRECORDED.getStyletype()) ||
...
//						}
	...
	private void thirdcancle(final int position) {
		...
		}else if(info.getType().equals(PaymentTypeEnum.BANK_CODE.getStyletype())){
			map.put("pay_type", ConstantData.PAYMODE_BY_BANKCODE+"");
		}else if(info.getType().equals(PaymentTypeEnum.YIPAY.getStyletype())){
