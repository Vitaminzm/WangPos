...
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?) and money >= 0 and personid = ?", new String[] { format.format(new Date(date)), personid }, "name", null, null);
//			}else if(ConstantData.LOG_RETURN == condition) {
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?) and money < 0 and personid = ?", new String[] { format.format(new Date(date)), personid }, "name", null, null);
//			}else if(ConstantData.LOG_ALL == condition) {
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?) and personid = ?", new String[] { format.format(new Date(date)), personid }, "name", null, null);
//			}
//		}else {
//			if(ConstantData.LOG_SALE == condition) {
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?) and money >= 0", new String[] { format.format(new Date(date)) }, "name", null, null);
//			}else if(ConstantData.LOG_RETURN == condition) {
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?) and money < 0", new String[] { format.format(new Date(date)) }, "name", null, null);
//			}else if(ConstantData.LOG_ALL == condition) {
//				cursor = db.query("pay_type", new String[] {"name", "sum(money) AS total"}, "date(skrq) = date(?)", new String[] { format.format(new Date(date)) }, "name", null, null);
//			}
//		}
//		while(cursor.moveToNext()) {
