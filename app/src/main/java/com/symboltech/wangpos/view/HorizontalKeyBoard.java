...
import com.symboltech.wangpos.myinterface.KeyBoardListener;
...
public class HorizontalKeyBoard extends Dialog implements OnClickListener, OnTouchListener, OnDismissListener {
	...
	int isActivity = 0; //针对于那种dialog样式的activity 0代表不是activity
	/**
	 * 
	 * @param context 上下文
	...
	@Override
	public void show() {
		final HorizontalKeyBoard lastBoard = MyApplication.getLastBoard();
		if(lastBoard != null && lastBoard != this && lastBoard.isShowing()) {
			try {
		...
		MyApplication.setLastBoard(this);
		if(null != mContentView){
			  ...
			  if(mObject instanceof Dialog) {
				  ...
				  if(flag == FLAG_EDIT) {
					  ...
					  LogUtil.i("length","length edit"+length);
				   }else if(flag == FLAG_TEXT) {
					  length=(int)((pos[1]+textView.getMeasuredHeight()-outRect.top)-(mScreenHeight-height));
					  LogUtil.i("length","length text"+length);
				  }
			  }
			  if(isActivity == 1){
				  LogUtil.i("length","activity"+length);
				  length=(int)((pos[1]+mWindow.getDecorView().getHeight()-outRect.top)-(mScreenHeight-height));
			  }
			  if(length>0){
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN) {
			if(editText != null && editText.getId() == v.getId()) {
				EditText lastEditText = MyApplication.getLastEditText();
				if(lastEditText != null && lastEditText == editText && isShowing()) {
					return true;
				}
				MyApplication.setLastEditText(editText);
				...
				TextView lastTextView = MyApplication.getLastTextView();
				if(lastTextView != null && lastTextView == textView && isShowing()) {
					return true;
				}
				MyApplication.setLastTextView(textView);
